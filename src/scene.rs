use crate::{random, Cube, Material, Ray, Scalar, Sphere, Vec3, AABB};
...
impl Scene {
	...
	pub fn create_sample_scene() -> Self {
		...
		let sun_mat = Material::Lambertain { albedo: Vec3::new(0.8, 0.4, 0.2), emission: 60.0 };
		...
		for a in -11..11 {
			for b in -11..11 {
				...
				let center = Vec3::new(a as Scalar + 0.9 * random(0.0, 1.0, &mut rand), 0.2, b as Scalar + 0.9 * random(0.0, 1.0, &mut rand));

				if (center - Vec3::new(4.0, 0.2, 0.0)).length() > 0.9 {
					if random_mat < 0.35 {
						// diffuse
						let albedo = Vec3::random(0.0, 1.0, &mut rand) * Vec3::random(0.0, 1.0, &mut rand);
